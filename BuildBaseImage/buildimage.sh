...
push_image()
{
    log "INFO: Login to the docker registry"
        /usr/bin/docker login -u bhanuprakashece410 -p $paswd
    ...
    /usr/bin/docker tag ${Build_Img} bhanuprakashece410/${Build_Img}
    /usr/bin/docker push bhanuprakashece410/${Build_Img}
